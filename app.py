...
def clear_blocklist(instance):
    logger.info(f"Starting blocklist clearance for {instance['url']}")
    ...
    try:
        ...
        logger.error(f"HTTP error occurred while clearing blocklist on {instance['url']}: {http_err}")
    except requests.exceptions.RequestException as req_err:
        logger.error(f"Request error occurred while clearing blocklist on {instance['url']}: {req_err}")
    except Exception as e:
